...
class EssayModel(torch.nn.Module):   # baseline model using bert with one output
    def __init__(self):
        super(EssayModel, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-uncased')
        self.fc = torch.nn.Linear(768, 1)  # 768 is the hidden size of BERT, 4 is the number of scores to predict
...
class EssayModel_multiop(torch.nn.Module):   # bert model with multi-head output
    def __init__(self):
        super(EssayModel_multiop, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-uncased')
...
class EssayModel3v9(torch.nn.Module):  # model 2 - changed embedding dimension to include manual features
    def __init__(self, additional_feature_dim, manual_feature_dim):
        super(EssayModel3v9, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-uncased')
        
        # Adding a dense layer after concatenation
        self.fc1 = torch.nn.Linear(768 + additional_feature_dim, 512)  # First dense layer
        self.fc2 = torch.nn.Linear(512  + manual_feature_dim, 4)  # Final output layer for regression

        self.relu = torch.nn.ReLU()  # Activation function
        self.dropout = torch.nn.Dropout(0.3)  # Dropout for regularization

    def forward(self, ids, mask, additional_features, manual_features):
        outputs = self.bert(ids, attention_mask=mask)
        pooled_output = outputs[1]  # CLS representation
        ...
        combined_output = torch.cat((x, manual_features), dim=1)

        # Final output layer
        x = self.fc2(combined_output)
        
        
        return x, combined_output

class EssayModel3v10(torch.nn.Module):  # model 2 - changed embedding dimension to include manual features
    def __init__(self, additional_feature_dim, manual_feature_dim):
        super(EssayModel3v10, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-uncased')
        
        # Adding a dense layer after concatenation
        self.fc1 = torch.nn.Linear(768 + additional_feature_dim, 768)  # First dense layer
        self.fc2 = torch.nn.Linear(768  + manual_feature_dim, 512)  
