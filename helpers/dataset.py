...
class EssayDataset_baseline(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'score' in row:
            labels = row[['score']].values.astype(float)
            return {
...
class EssayDataset_multiop(Dataset):
    def __init__(self, df, tokenizer, max_len=384):
        self.df = df
        self.tokenizer = tokenizer
        self.max_len = max_len
    ...
    def __getitem__(self, index):
        ...
        if 'AC' in row:
            ...
            return {
                ...
                'mask': torch.tensor(mask, dtype=torch.long),
                'labels': torch.tensor(labels, dtype=torch.float),
                'additional_features': torch.tensor(avg_sentence_embedding, dtype=torch.float),
                'manual_features': torch.tensor(manual_features, dtype=torch.float),
                'unique_id': row['Unique_ID']
            }
        else:
