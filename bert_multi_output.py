...
from helpers.dataset import EssayDataset_multiop
from helpers.model import EssayModel_multiop
...
# nohup python3 -u bert_multi_output.py --question Q1 >output/logs/log_bert_multi_output_q1.out 2>&1 &
# nohup python3 -u bert_multi_output.py --question Q2 >output/logs/log_bert_multi_output_q2.out 2>&1 &
...
test_dataset = EssayDataset_multiop(test_df_sub, tokenizer)
...
if not TRAINMODE:
    ...
    for seed in CFG.seeds:
        ...
        dt.to_csv(prediction_path + f'test_{Question}_bertbase{variant}_seed{seed}.csv', index=False)
        ...
        for col in cols:
            ...
            plt.savefig(os.path.join(figure_path, f'{col} confusion matrix for {Question} Seed {seed}.png'))
            plt.close()
