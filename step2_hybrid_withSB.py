...
if not TRAINMODE:
    ...
    model.load_state_dict(torch.load(save_path + f'bert_hybridsb_model_{Question}.pt', map_location=torch.device('cpu')))
    model.to(device)
else:
    ...
    for seed in CFG.seeds:
        ...
        train_dataset = EssayDataset2sb(train, sentence_embedding_train, tokenizer)
        train_loader = DataLoader(train_dataset, batch_size=CFG.train_batch_size, shuffle=True)

        val_dataset = EssayDataset2sb(val, sentence_embedding_train, tokenizer)
        val_loader = DataLoader(val_dataset, batch_size=CFG.train_batch_size, shuffle=True)

        model = EssayModel2v2(sentence_embedding_dim).to(device)
        ...
        #torch.save(model.state_dict(), save_path + f'hybridsb_model_{Question}_{variant}_seed{seed}.pt')
        for i in range(4):
        ...
        test_predictions_float.to_csv(prediction_path + f'test_{Question}_sb{variant}_seed{seed}_float.csv', index=False)
        ...
        dt.to_csv(prediction_path + f'test_{Question}_sb{variant}_seed{seed}.csv', index=False)
        ...
        performance_log = pd.concat([performance_log, log.to_frame().T], ignore_index=True)
        performance_log.to_csv(save_path + f'hybridsb_performance_log_{Question}_{variant}.csv', index=False) 
        if seed==189 and Question=="Q1":
            torch.save(model.state_dict(), save_path + f'hybridsb_model_{Question}_{variant}_seed{seed}.pt')
        if seed==589 and Question=="Q2":
            torch.save(model.state_dict(), save_path + f'hybridsb_model_{Question}_{variant}_seed{seed}.pt')
